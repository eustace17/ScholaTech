...
class TeacherActivity : AppCompatActivity() {
    ...
    lateinit var users : ArrayList<Teacher>
    lateinit var adapter: TeacherAdapter
    lateinit var progressDialog : ProgressDialog
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_teacher)
        listUsers = findViewById(R.id.mListTeachers)
        users = ArrayList()
        adapter = TeacherAdapter(this,users)
        ...
        var ref = FirebaseDatabase.getInstance().getReference().child("teachers")
        ...
        ref.addValueEventListener(object : ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (data in snapshot.children){
                    var user = data.getValue(Teacher::class.java)
                    users.add(user!!)
                }
